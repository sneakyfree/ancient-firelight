...
const W=1280,H=720;
// v2 palette — ember warmth now sits against real cold, so the fire reads as fire
...
function vignette(ctx){
  // lighter than v1 — it was crushing the type
...
function grain(ctx,t){
  // faint warm flicker; kept low so it never washes over text
...
/* text helpers — every line gets a dark halo so it stays legible over ANY background.
   This is the contrast fix: type never fights the painting behind it. */
function line(ctx,txt,x,y,size,color,al,{italic=false,weight=400,font=SERIF,align='center'}={}){
  ...
  // two-pass shadow: wide soft halo, then a tight one, then the glyph
  ctx.fillStyle=color;
...
/* Van Gogh pass — swirling directional strokes that give the frame texture and motion.
   Cheap to draw, and it's what makes it read as painted instead of empty. */
...
/* crude glyphs for the "you can't ___" litany — keeps the cold open alive */
...
function sceneColdOpen(ctx,t){
  // v2: we open ON a living campfire. The channel is called Ancient Firelight —
  // the fire is the first thing you see, not something you wait 37 seconds for.
  const dusk=1-ramp(t,0,9);                       // last light draining from the sky
  const fx=W*0.5,fy=H*0.74;
  let g=ctx.createLinearGradient(0,0,0,H);
  g.addColorStop(0,mix(NIGHT,'#4A2412',dusk*0.75));
  g.addColorStop(0.42,mix('#0B1219','#2A1610',dusk*0.55));
  g.addColorStop(1,mix('#080B0F','#150A06',0.5));
  ctx.fillStyle=g;ctx.fillRect(0,0,W,H);
  // painterly sky — swirling Van Gogh strokes, cold above, warm near the horizon
  strokes(ctx,t,150,[NIGHT2,COOL,'#16283A'],0.17,0,H*0.66,1.15);
  strokes(ctx,t,70,['#5A2A15',EMBER,'#7A3A1C'],0.09+0.13*dusk,H*0.30,H*0.80,1.4);
  // the setting sun sinking through the first 9 seconds
  if(dusk>0.01){const sy=H*0.42+130*(1-dusk);
    const sg=ctx.createRadialGradient(W*0.5,sy,4,W*0.5,sy,240);
    sg.addColorStop(0,rgba('#F0A05A',0.42*dusk));
    sg.addColorStop(0.35,rgba(EMBER,0.22*dusk));
    sg.addColorStop(1,'rgba(20,16,14,0)');ctx.fillStyle=sg;ctx.fillRect(0,0,W,H);}
  // stars, cool moonlit tone
  const starA=ramp(t,6,14)*0.85;
  if(starA>0.01){for(let i=0;i<110;i++){const sx=rnd(i)*W,sy=rnd(i*3.7)*H*0.58;
    const tw=0.5+0.5*Math.sin(t*1.5+i);ctx.globalAlpha=starA*(0.25+0.6*rnd(i*1.3))*tw;
    ctx.fillStyle=MOON;ctx.beginPath();ctx.arc(sx,sy,rnd(i*2.1)*1.4+0.35,0,6.2832);ctx.fill();}ctx.globalAlpha=1;}
  // ---- the campfire, burning from frame one ----
  const flare=1+0.55*ramp(t,37.2,40.6);           // it FLARES on "you huddled around a fire"
  fireGlow(ctx,fx,fy,0.92*flare,t);
  smoke(ctx,t,fx,fy,1.0,0.95);
  // ring of figures around it, resolving as the fire grows
  const seatA=ramp(t,2.5,7.0);
  if(seatA>0.01){ctx.globalAlpha=seatA*0.9;
    figureLying(ctx,fx-300,fy+40,0.85,'#0A0706');
    figure(ctx,fx-150,fy+10,0.92,'#0A0706');
    figure(ctx,fx+152,fy+10,0.92,'#0A0706');ctx.globalAlpha=1;}
  const figA=ramp(t,38.5,42);
  if(figA>0.01){ctx.globalAlpha=figA;
    figure(ctx,fx+250,fy+16,0.85,'#0A0706');
    figureLying(ctx,fx+320,fy+42,0.8,'#0A0706');ctx.globalAlpha=1;}
  flame(ctx,fx,fy,0.95*flare+0.09*Math.sin(t*2.1),t);
  emberDrift(ctx,t,60,0.95,fx,fy);
  // ---- the "you can't ___" litany, one crude glyph per denial ----
  const LIT=[['work',4.72],['forage',7.14],['hunt',8.98],['dig',10.96],['walk',14.68],['hand',18.32]];
  for(let i=0;i<LIT.length;i++){
    const [kind,tw]=LIT[i];
    const a=env(t,tw-0.45,tw+0.15,tw+1.5,tw+2.3);
    if(a<0.01)continue;
    const gx=W*0.5+(i-2.5)*168, gy=H*0.26;
    glyph(ctx,kind,gx,gy,1.25,a*0.95,ASH);
    ctx.save();ctx.globalAlpha=a*0.85;ctx.strokeStyle=rgba(EMBER,0.9);ctx.lineWidth=3;
    const sw=ramp(t,tw+0.35,tw+0.95);
    if(sw>0.01){ctx.beginPath();ctx.moveTo(gx-30,gy+30*sw-15);ctx.lineTo(gx-30+60*sw,gy-15);ctx.stroke();}
    ctx.restore();ctx.globalAlpha=1;
  }
  // "Half."
  ...
  vignette(ctx);
  {const flr=(0.030+0.022*Math.sin(t*17));ctx.fillStyle=`rgba(150,60,25,${flr})`;ctx.fillRect(0,0,W,H);}
...
function abstractQuote(ctx,t){
  ...
  if(mn>0.01)line(ctx,'This study is about MOONLIGHT — zero fire data.',W*0.5,boxY+256,27,EMBER2,mn,{italic:true});
...
function citationDrift(ctx,t){
  ...
  if(ar2>0.01){arrow(ctx,xs[1]+100,y,xs[2]-100,y,ar2,EMBER);
    line(ctx,'…now, a fact',(xs[1]+xs[2])/2,y-84,17,EMBER2,ar2,{italic:true,font:MONO});}
...
function eklundList(ctx,t){
  ...
  if(fireX>0.01){
    ...
    line(ctx,'not on the list',W*0.5,y+40,25,GOLD,fireX,{italic:true});ctx.globalAlpha=1;
  }
  citation(ctx,'Eklund A et al. 2017, Sci Rep 7:2097',ramp(t,224,226));
}
function scenePredators(ctx,t){
  baseBg(ctx,t,0.18);
  // --- ghost beasts surface faintly behind the firelight, one at a time, off the data beats ---
  // "night was lethal, big cats hunted in the dark"
  ghost(ctx,'lions',t,env(t,74.5,77.5,84,87)*0.13,0.2);
  ghost(ctx,'bear', t,env(t,84,86,90,92)*0.10,1.1);
  // "a real study about lions and darkness" (fades out before the moon graph at 116)
  ghost(ctx,'lions',t,env(t,102,105,113.5,115.5)*0.12,3.7);
  // the Hadza-in-lion-country beat (text-light)
  ghost(ctx,'lions',t,env(t,193,196,211,214)*0.11,5.1);
...
// signal readiness only once the typeface AND the ghost images are available
window.ready=false;
(function(){
  const names=['lions','bear','rhino','horses','aurochs','mammoth','megaloceros'];
  const imgs=names.map(n=>{const im=new Image();im.src='./ghosts_glow/'+n+'.png';GHOSTS[n]=im;return im;});
  function imgsReady(){return imgs.every(im=>im.complete&&(im.naturalWidth>0||im.naturalWidth===0));}
